...
async def process_sort_channel_list(data, ipv6=False, father=None):
    ...
    async def limited_get_speed(url, headers, cache_key, origin, is_ipv6, ipv6_proxy, resolution, filter_resolution,
                                min_resolution,
                                timeout,
                                callback):
        try:
            async with semaphore, asyncio.timeout(60):
                return await get_speed(url, headers, cache_key, origin, is_ipv6=is_ipv6, ipv6_proxy=ipv6_proxy,
